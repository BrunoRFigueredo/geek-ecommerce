...
//TODO @QueryDslPredicate(root = Produto.class) Predicate filter
    @GetMapping("/")
    public ResponseEntity<Paginacao> getAll(@RequestParam(name = "filtro",required = false,defaultValue = "") String filtro,
        ...
        BooleanExpression filter = Strings.isEmpty(filtro) ? QCategoria.categoria.status.eq(Categoria.Status.ATIVO) : QCategoria.categoria.status.eq(Categoria.Status.ATIVO).and(QCategoria.categoria.descricao.containsIgnoreCase(filtro));
